...
```jsonc
{
  ...
  "credits": {                           // 额度口径，页面上的「首日可得」由它算出来
    ...
    "dailyCheckin": 25,                  // 每日签到额度，没有签到就填 null
...
- 额度口径在 [`scripts/lib/credits.mjs`](scripts/lib/credits.mjs)：站点接口只暴露邀请额度，注册基础额度和签到额度是站内公示、只能手工登记在 `credits` 里；`npm run build` 会拿登记值和接口实测值对账，不一致就告警，看到告警就去更新 `data/sites.json`。
